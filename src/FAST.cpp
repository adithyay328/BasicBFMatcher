...
FASTDetector::FASTDetector() {
  numIntens = NUMBEROFINTENSITIES;
  thresh = THRESH;
  radius = BRESRADIUS;
};

// Configurable constructor
FASTDetector::FASTDetector(int numIntensities, int threshVal) {
  numIntens = numIntensities;
  thresh = threshVal;
  radius = BRESRADIUS;
}

// Given a BGR image, extracts FAST corners and draws a circle on
// each one of those points
std::vector<cv::Point>* FASTDetector::detect(Mat image) {
  ...
  // convert into vector to return as result
  std::list<cv::Point> corners;
